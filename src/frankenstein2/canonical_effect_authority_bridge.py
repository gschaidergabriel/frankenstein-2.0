...
:class:`EffectCallIntent`, which deliberately has no effect id but, on the canonical
path, must carry the immutable semantic :class:`EffectRequestIdentity` that the
EntityOS EffectGate evaluates. A separately resolved canonical authority
implementation then returns typed evidence containing the authoritative effect id,
exact call identity, and the same semantic request digest.
...
response matches that already-resolved identity, the exact result-free call, and the
exact semantic request before anything reaches the executor.
...
class CanonicalEffectAuthorityIdentityError(CanonicalEffectAuthorityBridgeError):
    """The authority response is incomplete, stale, or belongs to another call/request."""
...
class EffectCallIntent:
    """Pre-authority call identity plus immutable semantic request identity.

    ``request`` remains optional only so older stored/constructed compatibility objects
    can still be represented and inspected. The canonical dispatch/binding path rejects
    a missing request *before* authority evaluation, so legacy correlation-only intent
    can never gain current semantic-authorization credit or reach the executor.
    """
...
class CanonicalEffectAuthorityEvidence:
    ...
    ``request_sha256`` binds the authority decision to the exact immutable semantic
    request it evaluated. Current canonical binding requires it for every decision,
    including non-ALLOW outcomes, because an unbound DENY/UNKNOWN is still not evidence
    about the caller's request.
    """
...
class CanonicalEffectBinding:
    """Result of validating canonical authority against one exact intent/request."""
...
def intent_from_prepared_candidate(prepared: EffectCallBinding) -> EffectCallIntent:
    ...
    If the prepared candidate already has semantic request identity, preserve it exactly.
    Legacy candidates without it remain representable but canonical dispatch rejects them.
    """
...
def _require_semantic_request(intent: EffectCallIntent) -> EffectRequestIdentity:
...
def _assert_same_call_and_request(
    intent: EffectCallIntent,
    evidence: CanonicalEffectAuthorityEvidence,
) -> None:
    request = _require_semantic_request(intent)
    expected = {
...
def bind_canonical_effect(
    ...
    *,
    ...
    """Bind only authority evidence for the exact call and semantic request."""
    ...
    _assert_same_call_and_request(intent, evidence)
    ...
    assert evidence.request_sha256 is not None
...
def dispatch_with_canonical_authority(
    ...
    *,
    ...
    """Evaluate one exact semantic request and dispatch only its exact pending ALLOW.

    Semantic request identity is required before ``authorize`` is called. The expected
    authority identity must already have been resolved by current project authority;
    this module cannot discover or admit it. DENY, REQUIRE_CONFIRMATION,
    DEGRADE_TO_PROPOSAL, UNKNOWN, authority failure, source-identity mismatch,
    call-identity mismatch and request-digest mismatch all stop before the executor.
    """
    if not isinstance(intent, EffectCallIntent):
        raise CanonicalEffectAuthorityIdentityError("INVALID_EFFECT_CALL_INTENT")
    _require_semantic_request(intent)
    if not isinstance(expected_authority, CanonicalEffectAuthorityIdentity):
        raise CanonicalEffectAuthorityIdentityError("EXPECTED_AUTHORITY_UNRESOLVED")
