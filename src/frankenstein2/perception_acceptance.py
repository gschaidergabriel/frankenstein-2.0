...
F2-WP-714 generation 1 — PREPARATION_ONLY.

This module evaluates evidence *about* the Perception Fabric.  It does not open devices,
perform capture, call a model/provider, persist raw frames, mutate UnifiedDB/world truth,
or mint effect/completion/whole-system authority.  In particular, a green synthetic or
repository-hosted run can at most become ELIGIBLE_FOR_FINAL_REVIEW: terminal perception
acceptance still requires the declared upstream component receipts and a non-synthetic local
real-device / OS-permission receipt.
"""
...
class UpstreamAcceptance:
    ...
    schema: ClassVar[str] = "FRANKENSTEIN2_PERCEPTION_UPSTREAM_ACCEPTANCE/v1"
...
class PerceptionAcceptanceAssessment:
    ...
    schema: ClassVar[str] = "FRANKENSTEIN2_PERCEPTION_ACCEPTANCE_ASSESSMENT/v1"
...
def assess_perception_acceptance(
    *,
    ...
    All five upstream workpackages must be represented exactly once.  All required cases must
    be represented exactly once.  Any FAIL dominates.  Missing/NOT_RUN evidence blocks.  The
    local-real-device case must be PASS and non-synthetic.  Even then, this preparation-only
    harness returns ELIGIBLE_FOR_FINAL_REVIEW rather than PASS.
    """
    ...
    dependency_blockers = tuple(
        sorted(set(REQUIRED_UPSTREAM_WORKPACKAGES) - set(upstream_ids))
    )
    ...
    failed_cases = tuple(
        sorted(item.case_id for item in cases if item.result == CASE_RESULT_FAIL)
    )
    ...
    if local_case is not None and local_case.result == CASE_RESULT_PASS and local_case.synthetic:
        case_blockers = tuple(
            sorted(set(case_blockers) | {"LOCAL_REAL_DEVICE_OS_PERMISSION_ACCEPTANCE:NON_SYNTHETIC_REQUIRED"})
        )
    ...
    elif dependency_blockers or case_blockers or not local_hardware_receipt_bound:
