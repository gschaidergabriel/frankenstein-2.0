"""Deterministic Familiarity / prediction-error binding for Frankenstein 2.0.

F2-WP-302 generation 1, provenance-fence repair.

This component binds an optional explicit PredictionResidual to one exact, already-built
WP-301 RetrievalPlan. It never accepts free-floating RetrievalResult values as familiarity
evidence. The caller must bind the expected plan digest plus need identity before a plan may
contribute an attention hint.

Authority invariants:

    FAMILIARITY != OBSERVATION
    RETRIEVAL_SCORE != TRUTH
    PREDICTION_MATCH != COMPLETION
    PREDICTION_MISMATCH != EFFECT_AUTHORITY
    CALLER_CONSTRUCTED_RESULT != ADMITTED_RETRIEVAL_EVIDENCE

A present residual decides MATCH versus MISMATCH mechanically. Familiarity can only
contribute a bounded attention hint and can never suppress a contradictory observation.
When no residual is available the result is UNKNOWN even if retrieved memories are strong.
"""
from __future__ import annotations

from dataclasses import asdict, dataclass
import hashlib
import json
import re
from typing import Any, Iterable

from .emergent_retrieval import (
    CLASSIFICATION_SELECTED,
    PLAN_CLASSIFICATION,
    PLAN_SCHEMA,
    RESULT_SCHEMA,
    RetrievalPlan,
    RetrievalResult,
)
from .memory_lifecycle import STATUS_ACTIVE, STATUS_DEGRADED
from .prediction_contract import PredictionResidual

BINDING_SCHEMA = "FRANKENSTEIN2_FAMILIARITY_PREDICTION_BINDING/v2"
SIGNAL_SCHEMA = "FRANKENSTEIN2_FAMILIARITY_PREDICTION_SIGNAL/v2"

STATUS_MATCH = "MATCH"
STATUS_MISMATCH = "MISMATCH"
STATUS_UNKNOWN = "UNKNOWN"

CLASSIFICATION = "FAMILIARITY_PREDICTION_CANDIDATE_SIGNAL_NOT_WORLD_TRUTH_OR_AUTHORITY"

MAX_BASIS_POINTS = 10_000
_MAX_IDENTIFIER_LENGTH = 512
_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")


class FamiliarityPredictionBindingError(ValueError):
    """Fail-closed F2-WP-302 contract error."""


def _identifier(name: str, value: Any) -> str:
    if not isinstance(value, str):
        raise FamiliarityPredictionBindingError(f"{name} must be a string")
    if not value or value != value.strip():
        raise FamiliarityPredictionBindingError(
            f"{name} must be non-empty and already trimmed"
        )
    if len(value) > _MAX_IDENTIFIER_LENGTH:
        raise FamiliarityPredictionBindingError(
            f"{name} exceeds {_MAX_IDENTIFIER_LENGTH} characters"
        )
    if any(ord(ch) < 0x20 or ord(ch) == 0x7F for ch in value):
        raise FamiliarityPredictionBindingError(f"{name} contains control characters")
    return value


def _generation(value: Any) -> int:
    if type(value) is not int or value < 1:
        raise FamiliarityPredictionBindingError("generation must be a positive integer")
    return value


def _sha256(name: str, value: Any) -> str:
    if not isinstance(value, str) or _SHA256_RE.fullmatch(value) is None:
        raise FamiliarityPredictionBindingError(
            f"{name} must be lowercase 64-hex SHA-256"
        )
    return value


def _refs(name: str, values: Iterable[str], *, allow_empty: bool = False) -> tuple[str, ...]:
    if isinstance(values, (str, bytes)):
        raise FamiliarityPredictionBindingError(f"{name} must be an iterable of references")
    refs = tuple(_identifier(name, value) for value in values)
    if not refs and not allow_empty:
        raise FamiliarityPredictionBindingError(f"{name} must contain at least one reference")
    if len(set(refs)) != len(refs):
        raise FamiliarityPredictionBindingError(f"{name} contains duplicate references")
    return tuple(sorted(refs))


def _canonical_json(value: Any) -> str:
    return json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=False)


def _digest(value: Any) -> str:
    return hashlib.sha256(_canonical_json(value).encode("utf-8")).hexdigest()


def _residual_error_bp(residual: PredictionResidual) -> int:
    denominator = max(
        residual.compared_leaf_count
        + len(residual.missing_paths)
        + len(residual.unexpected_paths),
        1,
    )
    return min(MAX_BASIS_POINTS, residual.mismatch_count * MAX_BASIS_POINTS // denominator)


def _basis_points(name: str, value: Any) -> int:
    if type(value) is not int or not (0 <= value <= MAX_BASIS_POINTS):
        raise FamiliarityPredictionBindingError(
            f"{name} must be an integer in [0, 10000]"
        )
    return value


def _validate_selected_result(result: RetrievalResult) -> tuple[str, int, str]:
    if not isinstance(result, RetrievalResult):
        raise FamiliarityPredictionBindingError(
            "retrieval plan selected entries must be RetrievalResult values"
        )
    if result.schema != RESULT_SCHEMA:
        raise FamiliarityPredictionBindingError("retrieval result schema mismatch")
    memory_id = _identifier("retrieval memory_id", result.memory_id)
    if type(result.memory_generation) is not int or result.memory_generation < 0:
        raise FamiliarityPredictionBindingError(
            "retrieval memory_generation must be a non-negative integer"
        )
    _sha256("memory_state_sha256", result.memory_state_sha256)
    _sha256("candidate_sha256", result.candidate_sha256)
    if result.lifecycle_status not in {STATUS_ACTIVE, STATUS_DEGRADED}:
        raise FamiliarityPredictionBindingError(
            "selected retrieval result must reference ACTIVE or DEGRADED memory"
        )
    if result.selected is not True or result.classification != CLASSIFICATION_SELECTED:
        raise FamiliarityPredictionBindingError(
            "retrieval plan selected entry violates WP301 selected classification"
        )
    _identifier("retrieval payload_ref", result.payload_ref)
    _sha256("retrieval payload_sha256", result.payload_sha256)
    _refs("retrieval provenance_ref", result.provenance_refs)
    if result.successor_ref is not None:
        raise FamiliarityPredictionBindingError(
            "selected retrieval result must not carry successor_ref"
        )

    axes = tuple(_identifier("overlap_axis", axis) for axis in result.overlap_axes)
    if len(set(axes)) != len(axes):
        raise FamiliarityPredictionBindingError("retrieval overlap_axes contain duplicates")
    if type(result.overlap_count) is not int or result.overlap_count != len(axes):
        raise FamiliarityPredictionBindingError("retrieval overlap_count/axes mismatch")
    if result.overlap_count < 1:
        raise FamiliarityPredictionBindingError(
            "selected retrieval result requires positive overlap"
        )

    signal_scores = tuple(result.signal_scores_bp)
    score_axes: list[str] = []
    positive_axes: list[str] = []
    for pair in signal_scores:
        if not isinstance(pair, tuple) or len(pair) != 2:
            raise FamiliarityPredictionBindingError("invalid retrieval signal score entry")
        axis = _identifier("signal axis", pair[0])
        score = _basis_points("signal score_bp", pair[1])
        score_axes.append(axis)
        if score > 0:
            positive_axes.append(axis)
    if len(set(score_axes)) != len(score_axes):
        raise FamiliarityPredictionBindingError("retrieval signal axes contain duplicates")
    if tuple(positive_axes) != axes:
        raise FamiliarityPredictionBindingError(
            "retrieval overlap axes do not match positive signal axes"
        )

    evidence_pairs = tuple(result.signal_evidence_refs)
    evidence_axes: list[str] = []
    for pair in evidence_pairs:
        if not isinstance(pair, tuple) or len(pair) != 2:
            raise FamiliarityPredictionBindingError("invalid retrieval signal evidence entry")
        axis = _identifier("signal evidence axis", pair[0])
        _refs("signal evidence_ref", pair[1])
        evidence_axes.append(axis)
    if tuple(evidence_axes) != tuple(score_axes):
        raise FamiliarityPredictionBindingError(
            "retrieval signal score/evidence axes mismatch"
        )

    weighted_score = _basis_points("retrieval weighted_score_bp", result.weighted_score_bp)
    _basis_points("retrieval bottleneck_score_bp", result.bottleneck_score_bp)
    if type(result.rank_score) is not int or result.rank_score < 0:
        raise FamiliarityPredictionBindingError(
            "retrieval rank_score must be a non-negative integer"
        )
    return memory_id, weighted_score, _digest(result.as_dict())


def _validate_retrieval_plan(
    plan: RetrievalPlan,
    *,
    expected_plan_sha256: str,
    expected_need_id: str,
    expected_need_sha256: str,
) -> tuple[tuple[str, int, str], ...]:
    if not isinstance(plan, RetrievalPlan):
        raise FamiliarityPredictionBindingError("retrieval_plan must be a RetrievalPlan")
    if plan.schema != PLAN_SCHEMA:
        raise FamiliarityPredictionBindingError("retrieval plan schema mismatch")
    if plan.classification != PLAN_CLASSIFICATION:
        raise FamiliarityPredictionBindingError("retrieval plan classification mismatch")
    if plan.need_id != expected_need_id:
        raise FamiliarityPredictionBindingError("retrieval plan need_id fence mismatch")
    if plan.need_sha256 != expected_need_sha256:
        raise FamiliarityPredictionBindingError("retrieval plan need digest fence mismatch")
    _sha256("retrieval plan need_sha256", plan.need_sha256)
    if plan.sha256() != expected_plan_sha256:
        raise FamiliarityPredictionBindingError("retrieval plan digest fence mismatch")
    if type(plan.candidate_count) is not int or plan.candidate_count < 0:
        raise FamiliarityPredictionBindingError(
            "retrieval plan candidate_count must be a non-negative integer"
        )
    if plan.candidate_count != len(plan.selected) + len(plan.not_selected):
        raise FamiliarityPredictionBindingError(
            "retrieval plan candidate_count/result count mismatch"
        )

    rows = tuple(_validate_selected_result(result) for result in plan.selected)
    memory_ids = [row[0] for row in rows]
    if len(set(memory_ids)) != len(memory_ids):
        raise FamiliarityPredictionBindingError(
            "retrieval plan contains duplicate selected memory_id"
        )
    return tuple(sorted(rows, key=lambda row: row[0]))


@dataclass(frozen=True, slots=True)
class FamiliarityPredictionSignal:
    schema: str
    prediction_id: str
    generation: int
    status: str
    residual_sha256: str | None
    observation_id: str | None
    observation_fingerprint_sha256: str | None
    prediction_error_bp: int | None
    familiarity_bp: int
    attention_priority_bp: int
    retrieval_plan_sha256: str | None
    retrieval_need_id: str | None
    retrieval_need_sha256: str | None
    retrieval_memory_ids: tuple[str, ...]
    retrieval_result_sha256s: tuple[str, ...]
    evidence_refs: tuple[str, ...]
    classification: str = CLASSIFICATION

    def as_dict(self) -> dict[str, Any]:
        return asdict(self)

    def canonical_json(self) -> str:
        return _canonical_json(self.as_dict())

    def sha256(self) -> str:
        return hashlib.sha256(self.canonical_json().encode("utf-8")).hexdigest()


@dataclass(frozen=True, slots=True, init=False)
class FamiliarityPredictionBinding:
    schema: str
    prediction_id: str
    generation: int
    expected_residual_sha256: str | None
    expected_retrieval_plan_sha256: str | None
    expected_retrieval_need_id: str | None
    expected_retrieval_need_sha256: str | None
    evidence_refs: tuple[str, ...]

    def __init__(
        self,
        *,
        schema: str,
        prediction_id: str,
        generation: int,
        expected_residual_sha256: str | None,
        expected_retrieval_plan_sha256: str | None,
        expected_retrieval_need_id: str | None,
        expected_retrieval_need_sha256: str | None,
        evidence_refs: Iterable[str],
    ) -> None:
        if schema != BINDING_SCHEMA:
            raise FamiliarityPredictionBindingError("binding schema mismatch")
        prediction_id = _identifier("prediction_id", prediction_id)
        generation = _generation(generation)
        if expected_residual_sha256 is not None:
            expected_residual_sha256 = _sha256(
                "expected_residual_sha256", expected_residual_sha256
            )

        plan_fence_values = (
            expected_retrieval_plan_sha256,
            expected_retrieval_need_id,
            expected_retrieval_need_sha256,
        )
        if any(value is None for value in plan_fence_values) and any(
            value is not None for value in plan_fence_values
        ):
            raise FamiliarityPredictionBindingError(
                "retrieval plan digest, need_id and need digest fences must be supplied together"
            )
        if expected_retrieval_plan_sha256 is not None:
            expected_retrieval_plan_sha256 = _sha256(
                "expected_retrieval_plan_sha256", expected_retrieval_plan_sha256
            )
            expected_retrieval_need_id = _identifier(
                "expected_retrieval_need_id", expected_retrieval_need_id
            )
            expected_retrieval_need_sha256 = _sha256(
                "expected_retrieval_need_sha256", expected_retrieval_need_sha256
            )
        refs = _refs("binding evidence_ref", evidence_refs)

        object.__setattr__(self, "schema", schema)
        object.__setattr__(self, "prediction_id", prediction_id)
        object.__setattr__(self, "generation", generation)
        object.__setattr__(self, "expected_residual_sha256", expected_residual_sha256)
        object.__setattr__(self, "expected_retrieval_plan_sha256", expected_retrieval_plan_sha256)
        object.__setattr__(self, "expected_retrieval_need_id", expected_retrieval_need_id)
        object.__setattr__(self, "expected_retrieval_need_sha256", expected_retrieval_need_sha256)
        object.__setattr__(self, "evidence_refs", refs)

    @classmethod
    def create(
        cls,
        *,
        prediction_id: str,
        generation: int,
        expected_residual_sha256: str | None,
        expected_retrieval_plan_sha256: str | None = None,
        expected_retrieval_need_id: str | None = None,
        expected_retrieval_need_sha256: str | None = None,
        evidence_refs: Iterable[str],
    ) -> "FamiliarityPredictionBinding":
        return cls(
            schema=BINDING_SCHEMA,
            prediction_id=prediction_id,
            generation=generation,
            expected_residual_sha256=expected_residual_sha256,
            expected_retrieval_plan_sha256=expected_retrieval_plan_sha256,
            expected_retrieval_need_id=expected_retrieval_need_id,
            expected_retrieval_need_sha256=expected_retrieval_need_sha256,
            evidence_refs=evidence_refs,
        )

    def evaluate(
        self,
        *,
        residual: PredictionResidual | None,
        retrieval_plan: RetrievalPlan | None = None,
    ) -> FamiliarityPredictionSignal:
        if retrieval_plan is None:
            if self.expected_retrieval_plan_sha256 is not None:
                raise FamiliarityPredictionBindingError(
                    "expected retrieval plan fence was supplied but retrieval_plan is unavailable"
                )
            result_rows: tuple[tuple[str, int, str], ...] = ()
            retrieval_plan_sha256 = None
            retrieval_need_id = None
            retrieval_need_sha256 = None
        else:
            if self.expected_retrieval_plan_sha256 is None:
                raise FamiliarityPredictionBindingError(
                    "present retrieval_plan requires explicit plan and need identity fences"
                )
            result_rows = _validate_retrieval_plan(
                retrieval_plan,
                expected_plan_sha256=self.expected_retrieval_plan_sha256,
                expected_need_id=self.expected_retrieval_need_id,
                expected_need_sha256=self.expected_retrieval_need_sha256,
            )
            retrieval_plan_sha256 = retrieval_plan.sha256()
            retrieval_need_id = retrieval_plan.need_id
            retrieval_need_sha256 = retrieval_plan.need_sha256

        familiarity_bp = max((row[1] for row in result_rows), default=0)

        if residual is None:
            if self.expected_residual_sha256 is not None:
                raise FamiliarityPredictionBindingError(
                    "expected residual digest was supplied but residual is unavailable"
                )
            status = STATUS_UNKNOWN
            residual_sha256 = None
            observation_id = None
            observation_fingerprint_sha256 = None
            prediction_error_bp = None
            attention_priority_bp = familiarity_bp
        else:
            if not isinstance(residual, PredictionResidual):
                raise FamiliarityPredictionBindingError(
                    "residual must be PredictionResidual or None"
                )
            if residual.prediction_id != self.prediction_id:
                raise FamiliarityPredictionBindingError("prediction_id mismatch")
            if residual.generation != self.generation:
                raise FamiliarityPredictionBindingError("prediction generation mismatch")
            residual_sha256 = residual.sha256()
            if self.expected_residual_sha256 is None:
                raise FamiliarityPredictionBindingError(
                    "present residual requires expected_residual_sha256 fence"
                )
            if residual_sha256 != self.expected_residual_sha256:
                raise FamiliarityPredictionBindingError("residual digest mismatch")
            _identifier("observation_id", residual.observation_id)
            _sha256(
                "observation_fingerprint_sha256",
                residual.observation_fingerprint_sha256,
            )
            prediction_error_bp = _residual_error_bp(residual)
            status = STATUS_MATCH if residual.exact_match else STATUS_MISMATCH
            attention_priority_bp = max(familiarity_bp, prediction_error_bp)
            observation_id = residual.observation_id
            observation_fingerprint_sha256 = residual.observation_fingerprint_sha256

        return FamiliarityPredictionSignal(
            schema=SIGNAL_SCHEMA,
            prediction_id=self.prediction_id,
            generation=self.generation,
            status=status,
            residual_sha256=residual_sha256,
            observation_id=observation_id,
            observation_fingerprint_sha256=observation_fingerprint_sha256,
            prediction_error_bp=prediction_error_bp,
            familiarity_bp=familiarity_bp,
            attention_priority_bp=attention_priority_bp,
            retrieval_plan_sha256=retrieval_plan_sha256,
            retrieval_need_id=retrieval_need_id,
            retrieval_need_sha256=retrieval_need_sha256,
            retrieval_memory_ids=tuple(row[0] for row in result_rows),
            retrieval_result_sha256s=tuple(row[2] for row in result_rows),
            evidence_refs=self.evidence_refs,
        )


__all__ = [
    "BINDING_SCHEMA",
    "SIGNAL_SCHEMA",
    "STATUS_MATCH",
    "STATUS_MISMATCH",
    "STATUS_UNKNOWN",
    "CLASSIFICATION",
    "FamiliarityPredictionBinding",
    "FamiliarityPredictionBindingError",
    "FamiliarityPredictionSignal",
]
