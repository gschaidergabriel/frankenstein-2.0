...
but behavior-capable trial transport is intentionally minimal: only the
canonical mediated semantic JSON crosses the trial boundary. Source provenance,
semantic class, trial position, payload digests and witness/source identities
remain verifier-side. A factory-valid mediator plus byte-for-byte wire equality
is required before a MediatedSemanticState can be admitted, so self-consistent
forged wire bytes cannot reach behavioral execution through the admitted API.
The admitted behavior-capable state itself stores only canonical semantic JSON;
verifier/readback topology labels are validation inputs or post-execution
metadata and are not retained on the behavior-visible state object.
...
execution binds source -> verifier admission -> trial -> child identities.
"""
...
class MediatedSemanticState:
    canonical_semantic_json: str
    # Compatibility-only verifier inputs. InitVar values are validated during
    # admission/direct construction but are deliberately not retained as state
    # attributes or included in the factory seal/digest.
    semantic_sha256: InitVar[str | None] = None
    wire_sha256: InitVar[str | None] = None
    trial_process_identity: InitVar[str | None] = None
    _factory_seal: object | None = field(default=None, init=False, repr=False, compare=False)
    _factory_sha256: str | None = field(default=None, init=False, repr=False, compare=False)
    ...
    def __post_init__(
        ...
        trial_process_identity: str | None,
    ) -> None:
        semantic = _strict_json(self.canonical_semantic_json.encode("utf-8"))
        canonical = _canonical_json(semantic)
        if canonical != self.canonical_semantic_json:
        ...
        if semantic_sha256 is not None and _sha("semantic_sha256", semantic_sha256) != canonical_sha:
            raise G10MediatorError("semantic_sha256 mismatch")
        if wire_sha256 is not None:
            _sha("wire_sha256", wire_sha256)
        if trial_process_identity is not None:
            _text("trial_process_identity", trial_process_identity)
...
def admit_mediated_semantic_state(
    *,
    ...
    """Verifier-side admission; topology identity is checked but never retained on behavior state."""
...
def bind_independent_semantic_mediator_crossover(
    *,
    ...
    for mediator, state, receipt in zip(mediators, states, execution_receipts):
        ...
        state_semantic_sha = hashlib.sha256(state.canonical_semantic_json.encode("utf-8")).hexdigest()
        if state_semantic_sha != mediator.semantic_sha256:
    ...
    # These are verifier-side repository labels only. They intentionally do not
    # live on MediatedSemanticState and mint no target process-topology credit.
    verifier_trial_refs = tuple(f"verifier-trial:g10:{position}" for position in range(1, 5))
    candidate = IndependentSemanticMediatorCrossoverCandidate(
        ...
        wire_sha256s=tuple(
            hashlib.sha256(mediator.to_wire()).hexdigest() for mediator in mediators
        ),  # type: ignore[arg-type]
        source_process_identities=tuple(mediator.source_process_identity for mediator in mediators),  # type: ignore[arg-type]
        trial_process_identities=verifier_trial_refs,  # type: ignore[arg-type]
