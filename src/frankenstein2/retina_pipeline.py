"""Deterministic Retina frame/quality/delta/continuity boundary for F2-WP-700.
...
class RetinaFrameSignal:
    ...
    classification: ClassVar[str] = "RETINA_CALLER_MEASUREMENT_NOT_OBJECT_WORLD_TRUTH_OR_EFFECT_AUTHORITY"
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "continuity_epoch", _text("continuity_epoch", self.continuity_epoch))
        _micros("quality_micros", self.quality_micros)
        delta_fields = (self.delta_micros, self.delta_reference_frame_id, self.delta_reference_frame_sha256)
        if all(item is None for item in delta_fields):
            pass
        elif any(item is None for item in delta_fields):
            raise RetinaPipelineError("delta measurement and reference identity must be all present or all absent")
        else:
            _micros("delta_micros", self.delta_micros)
            object.__setattr__(self, "delta_reference_frame_id", _text("delta_reference_frame_id", self.delta_reference_frame_id))
            _sha256("delta_reference_frame_sha256", self.delta_reference_frame_sha256)
            if self.delta_reference_frame_id == self.frame_id:
                raise RetinaPipelineError("delta reference cannot point to the current frame")
...
class RetinaPolicy:
    ...
    classification: ClassVar[str] = "RETINA_THRESHOLD_POLICY_NOT_WORLD_TRUTH_EFFECT_OR_COMPLETION_AUTHORITY"
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.max_interframe_gap_ns, bool) or not isinstance(self.max_interframe_gap_ns, int) or self.max_interframe_gap_ns <= 0:
            raise RetinaPipelineError("max_interframe_gap_ns must be an integer > 0")
        object.__setattr__(self, "provenance_refs", _refs("provenance_refs", self.provenance_refs))
...
class RetinaAssessment:
    ...
    classification: ClassVar[str] = "RETINA_PERCEPT_EVENT_CANDIDATE_NOT_OBJECT_WORLD_TRUTH_GWT_EFFECT_OR_COMPLETION_AUTHORITY"
    _QUALITY = frozenset({"QUALITY_PASS", "QUALITY_REJECTED"})
    _DELTA = frozenset({"BASELINE", "NO_SALIENT_DELTA", "SALIENT_DELTA"})
    _CONTINUITY = frozenset({"BASELINE", "CONTINUOUS", "BREAK_STREAM", "BREAK_EPOCH", "BREAK_GENERATION_GAP", "BREAK_TIME_GAP"})

    def __post_init__(self) -> None:
        object.__setattr__(self, "assessment_id", _text("assessment_id", self.assessment_id))
        object.__setattr__(self, "current_frame_id", _text("current_frame_id", self.current_frame_id))
        _sha256("current_frame_sha256", self.current_frame_sha256)
        _sha256("current_signal_sha256", self.current_signal_sha256)
        prev = (self.previous_frame_id, self.previous_frame_sha256, self.previous_signal_sha256)
        if all(item is None for item in prev):
            pass
        elif any(item is None for item in prev):
            raise RetinaPipelineError("previous frame identity fields must be all set or all absent")
        else:
            object.__setattr__(self, "previous_frame_id", _text("previous_frame_id", self.previous_frame_id))
        ...
        if not isinstance(self.percept_event_candidate, bool):
            raise RetinaPipelineError("percept_event_candidate must be bool")
        object.__setattr__(self, "event_reason", _text("event_reason", self.event_reason))
        object.__setattr__(self, "provenance_refs", _refs("provenance_refs", self.provenance_refs))
        eligible = self.quality_status == "QUALITY_PASS" and self.delta_status == "SALIENT_DELTA" and self.continuity_status == "CONTINUOUS"
        if self.percept_event_candidate != eligible:
            raise RetinaPipelineError("percept_event_candidate must equal the explicit quality/delta/continuity conjunction")
...
def _verify_signal(signal: RetinaFrameSignal, *, expected_sha256: str, name: str) -> None:
...
def assess_retina_transition(*, assessment_id: str, current: RetinaFrameSignal,
    expected_current_signal_sha256: str, policy: RetinaPolicy, expected_policy_sha256: str,
    previous: RetinaFrameSignal | None = None, expected_previous_signal_sha256: str | None = None,
    provenance_refs: tuple[str, ...]) -> RetinaAssessment:
    """Classify cheap frame measurements without reading or interpreting pixels."""
    _verify_signal(current, expected_sha256=expected_current_signal_sha256, name="current")
    _verify_policy(policy, expected_sha256=expected_policy_sha256)

    if previous is None:
        if expected_previous_signal_sha256 is not None:
            raise RetinaPipelineError("expected_previous_signal_sha256 must be absent when previous is absent")
        if current.delta_micros is not None:
            raise RetinaPipelineError("baseline frame must not claim a delta without a previous frame")
        ...
        if expected_previous_signal_sha256 is None:
            raise RetinaPipelineError("expected_previous_signal_sha256 is required with previous")
        _verify_signal(previous, expected_sha256=expected_previous_signal_sha256, name="previous")
        if current.delta_micros is None:
            raise RetinaPipelineError("non-baseline frame requires an explicit delta measurement")
        ...
        if current.captured_monotonic_ns <= previous.captured_monotonic_ns:
            raise RetinaPipelineError("current frame must be strictly newer by monotonic time")
        ...
        elif current.captured_monotonic_ns - previous.captured_monotonic_ns > policy.max_interframe_gap_ns:
            continuity_status = "BREAK_TIME_GAP"
        else:
            continuity_status = "CONTINUOUS"
        delta_status = "SALIENT_DELTA" if current.delta_micros >= policy.salient_delta_micros else "NO_SALIENT_DELTA"
        previous_frame_id = previous.frame_id
        previous_frame_sha256 = previous.frame_sha256
        previous_signal_sha256 = previous.sha256()

    quality_status = "QUALITY_PASS" if current.quality_micros >= policy.min_quality_micros else "QUALITY_REJECTED"
    percept_event_candidate = quality_status == "QUALITY_PASS" and delta_status == "SALIENT_DELTA" and continuity_status == "CONTINUOUS"
