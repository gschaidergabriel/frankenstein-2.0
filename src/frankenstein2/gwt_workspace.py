...
def _assert_selection_build_lineage(selection: WorkspaceSelection) -> None:
    if not isinstance(selection.selected, tuple) or not all(
        type(item) is SelectedCandidate for item in selection.selected
    ):
        raise GwtWorkspaceError("selection selected candidates must be concrete SelectedCandidate")
