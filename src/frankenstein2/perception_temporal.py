...
def _admission_refs(value: Any) -> tuple[tuple[str, str], ...]:
    ...
    for item in value:
        if type(item) is not tuple or len(item) != 2:
            raise PerceptionTemporalError('admitted_alignment_witness_refs items must be (witness_sha256, admission_sha256)')
        witness_sha, admission_sha = item
        checked.append((_sha256('admitted witness_sha256', witness_sha), _sha256('alignment admission_sha256', admission_sha)))
    witness_shas = [witness_sha for witness_sha, _ in checked]
    if len(witness_shas) != len(set(witness_shas)):
        raise PerceptionTemporalError('admitted_alignment_witness_refs must not repeat witness_sha256')
    admission_shas = [admission_sha for _, admission_sha in checked]
    if len(admission_shas) != len(set(admission_shas)):
        raise PerceptionTemporalError('admitted_alignment_witness_refs must not repeat admission_sha256')
...
class TemporalPerceptRef:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'schema': self.schema, 'classification': self.classification, 'ref_id': self.ref_id, 'source_id': self.source_id, 'source_sequence': self.source_sequence, 'source_generation': self.source_generation, 'clock_domain': self.clock_domain, 'source_time_ns': self.source_time_ns, 'reference_offset_ns': self.reference_offset_ns, 'reference_time_ns': self.reference_time_ns, 'clock_uncertainty_ns': self.clock_uncertainty_ns, 'max_freshness_ns': self.max_freshness_ns, 'observed_claim_sha256': self.observed_claim_sha256, 'world_truth_authority': 'NONE', 'provenance_refs': list(self.provenance_refs)}
...
class ClockAlignmentWitness:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'schema': self.schema, 'classification': self.classification, 'alignment_id': self.alignment_id, 'alignment_generation': self.alignment_generation, 'left_clock_domain': self.left_clock_domain, 'left_source_generation': self.left_source_generation, 'left_reference_offset_ns': self.left_reference_offset_ns, 'left_max_uncertainty_ns': self.left_max_uncertainty_ns, 'right_clock_domain': self.right_clock_domain, 'right_source_generation': self.right_source_generation, 'right_reference_offset_ns': self.right_reference_offset_ns, 'right_max_uncertainty_ns': self.right_max_uncertainty_ns, 'valid_from_reference_ns': self.valid_from_reference_ns, 'valid_through_reference_ns': self.valid_through_reference_ns, 'evidence_sha256': self.evidence_sha256, 'world_truth_authority': 'NONE', 'provenance_refs': list(self.provenance_refs)}

    def sha256(self) -> str:
        return _digest(self.as_dict())

    def matches_pair(self, left: TemporalPerceptRef, right: TemporalPerceptRef) -> bool:
        orientations = ((self.left_clock_domain, self.left_source_generation, self.left_reference_offset_ns, self.left_max_uncertainty_ns, self.right_clock_domain, self.right_source_generation, self.right_reference_offset_ns, self.right_max_uncertainty_ns), (self.right_clock_domain, self.right_source_generation, self.right_reference_offset_ns, self.right_max_uncertainty_ns, self.left_clock_domain, self.left_source_generation, self.left_reference_offset_ns, self.left_max_uncertainty_ns))
        for left_domain, left_generation, left_offset, left_uncertainty, right_domain, right_generation, right_offset, right_uncertainty in orientations:
            if left.clock_domain == left_domain and left.source_generation == left_generation and (left.reference_offset_ns == left_offset) and (left.clock_uncertainty_ns <= left_uncertainty) and (right.clock_domain == right_domain) and (right.source_generation == right_generation) and (right.reference_offset_ns == right_offset) and (right.clock_uncertainty_ns <= right_uncertainty) and (self.valid_from_reference_ns <= left.reference_time_ns <= self.valid_through_reference_ns) and (self.valid_from_reference_ns <= right.reference_time_ns <= self.valid_through_reference_ns):
                return True
        return False
...
class ObservationWindow:
    ...
    alignment_admission_refs: tuple[tuple[str, str], ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        checked_sources: list[tuple[str, str, str]] = []
        for item in self.source_refs:
            ...
            checked_sources.append((_text('source_id', source_id), _text('ref_id', ref_id), _sha256('ref_sha256', ref_sha)))
        if len({ref_id for _, ref_id, _ in checked_sources}) != len(checked_sources):
            raise PerceptionTemporalError('source_refs must not repeat ref_id')
        all_ids = groups[0] | groups[1] | groups[2]
        if all_ids != {ref_id for _, ref_id, _ in checked_sources}:
            raise PerceptionTemporalError('every source ref must have exactly one temporal disposition')
        object.__setattr__(self, 'source_refs', tuple(sorted(checked_sources)))
        ...
        checked_witnesses: list[tuple[str, str]] = []
        for item in self.alignment_witness_refs:
            ...
            checked_witnesses.append((_text('alignment_id', alignment_id), _sha256('witness_sha256', witness_sha)))
        if len({alignment_id for alignment_id, _ in checked_witnesses}) != len(checked_witnesses):
            raise PerceptionTemporalError('alignment_witness_refs must not repeat alignment_id')
        object.__setattr__(self, 'alignment_witness_refs', tuple(sorted(checked_witnesses)))
        checked_admissions = _admission_refs(self.alignment_admission_refs)
        witness_shas = {witness_sha for _, witness_sha in checked_witnesses}
        admitted_witness_shas = {witness_sha for witness_sha, _ in checked_admissions}
        if witness_shas != admitted_witness_shas:
            raise PerceptionTemporalError('alignment_admission_refs must bind every used alignment witness exactly once')
        object.__setattr__(self, 'alignment_admission_refs', checked_admissions)
        object.__setattr__(self, 'provenance_refs', _refs('provenance_refs', self.provenance_refs))
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'schema': self.schema, 'classification': self.classification, 'window_id': self.window_id, 'reference_now_ns': self.reference_now_ns, 'max_join_skew_ns': self.max_join_skew_ns, 'max_clock_uncertainty_ns': self.max_clock_uncertainty_ns, 'alignment_status': self.alignment_status, 'current_ref_ids': list(self.current_ref_ids), 'stale_ref_ids': list(self.stale_ref_ids), 'unaligned_ref_ids': list(self.unaligned_ref_ids), 'source_refs': [{'source_id': source_id, 'ref_id': ref_id, 'ref_sha256': ref_sha} for source_id, ref_id, ref_sha in self.source_refs], 'alignment_witness_refs': [{'alignment_id': alignment_id, 'witness_sha256': witness_sha} for alignment_id, witness_sha in self.alignment_witness_refs], 'alignment_admission_refs': [{'witness_sha256': witness_sha, 'admission_sha256': admission_sha} for witness_sha, admission_sha in self.alignment_admission_refs], 'arrival_order_is_event_time': False, 'same_grid_cycle_is_same_real_world_time': False, 'unknown_or_unaligned_preserved': True, 'unproven_cross_clock_is_unaligned': True, 'numeric_reference_offset_self_attests_alignment': False, 'caller_minted_witness_self_attests_admission': False, 'resolves_semantic_disagreement': False, 'world_truth_authority': 'NONE', 'effect_authority': 'NONE', 'completion_authority': 'NONE', 'provenance_refs': list(self.provenance_refs)}

    def sha256(self) -> str:
        return _digest(self.as_dict())

def bind_observed_claim(*, claim: EpistemicPerceptClaim, expected_claim_sha256: str, ref_id: str, source_id: str, source_sequence: int, clock_domain: str, reference_offset_ns: int, clock_uncertainty_ns: int, max_freshness_ns: int, provenance_refs: tuple[str, ...]) -> TemporalPerceptRef:
    ...
    return TemporalPerceptRef(ref_id=ref_id, source_id=source_id, source_sequence=source_sequence, source_generation=claim.source_generation, clock_domain=clock_domain, source_time_ns=claim.source_time_ns, reference_offset_ns=reference_offset_ns, clock_uncertainty_ns=clock_uncertainty_ns, max_freshness_ns=max_freshness_ns, observed_claim_sha256=expected_claim_sha256, provenance_refs=tuple(sorted(provenance)))

def _matching_alignment_witness(a: TemporalPerceptRef, b: TemporalPerceptRef, *, alignment_witnesses: tuple[ClockAlignmentWitness, ...], admission_by_witness_sha: dict[str, str]) -> tuple[ClockAlignmentWitness, str] | None:
    matches = [witness for witness in alignment_witnesses if witness.matches_pair(a, b)]
    if len(matches) != 1:
        return None
    witness = matches[0]
    witness_sha = witness.sha256()
    admission_sha = admission_by_witness_sha.get(witness_sha)
    if admission_sha is None:
        return None
    return (witness, admission_sha)

def _pair_is_admissible(a: TemporalPerceptRef, b: TemporalPerceptRef, *, max_join_skew_ns: int, alignment_witnesses: tuple[ClockAlignmentWitness, ...], admission_by_witness_sha: dict[str, str]) -> tuple[bool, ClockAlignmentWitness | None, str | None]:
    ...
    if a.clock_identity == b.clock_identity:
        ...
        match = _matching_alignment_witness(a, b, alignment_witnesses=alignment_witnesses, admission_by_witness_sha=admission_by_witness_sha)
        if match is None:
            return (False, None, None)
        witness, admission_sha = match
    worst_case_separation = abs(a.reference_time_ns - b.reference_time_ns) + a.clock_uncertainty_ns + b.clock_uncertainty_ns
    return (worst_case_separation <= max_join_skew_ns, witness, admission_sha)

def build_observation_window(*, refs: tuple[TemporalPerceptRef, ...], reference_now_ns: int, max_join_skew_ns: int, max_clock_uncertainty_ns: int, provenance_refs: tuple[str, ...], alignment_witnesses: tuple[ClockAlignmentWitness, ...]=(), admitted_alignment_witness_refs: tuple[tuple[str, str], ...]=()) -> ObservationWindow:
    ...
    if type(alignment_witnesses) is not tuple or any((type(item) is not ClockAlignmentWitness for item in alignment_witnesses)):
        raise PerceptionTemporalError('alignment_witnesses must be an immutable tuple of concrete ClockAlignmentWitness values')
    ...
    admitted_refs = _admission_refs(admitted_alignment_witness_refs)
    admission_by_witness_sha = dict(admitted_refs)
    ...
    used_witnesses: dict[str, tuple[ClockAlignmentWitness, str]] = {}
    for index, left in enumerate(current):
        for right in current[index + 1:]:
            admissible, witness, admission_sha = _pair_is_admissible(left, right, max_join_skew_ns=max_join_skew_ns, alignment_witnesses=alignment_witnesses, admission_by_witness_sha=admission_by_witness_sha)
            if not admissible:
                incompatible_ids.add(left.ref_id)
                incompatible_ids.add(right.ref_id)
            elif witness is not None and admission_sha is not None:
                used_witnesses[witness.alignment_id] = (witness, admission_sha)
    if incompatible_ids:
    ...
    admission_refs: list[tuple[str, str]] = []
    for witness, admission_sha in used_witnesses.values():
        witness_sha = witness.sha256()
        witness_refs.append((witness.alignment_id, witness_sha))
        admission_refs.append((witness_sha, admission_sha))
        provenance.update(witness.provenance_refs)
        provenance.add(f'clock-alignment-witness-sha256:{witness_sha}')
        provenance.add(f'clock-alignment-admission-sha256:{admission_sha}')
    payload = {'reference_now_ns': reference_now_ns, 'max_join_skew_ns': max_join_skew_ns, 'max_clock_uncertainty_ns': max_clock_uncertainty_ns, 'current_ref_ids': sorted((item.ref_id for item in current)), 'stale_ref_ids': sorted((item.ref_id for item in stale)), 'unaligned_ref_ids': sorted((item.ref_id for item in unaligned)), 'source_refs': sorted(source_refs), 'alignment_witness_refs': sorted(witness_refs), 'alignment_admission_refs': sorted(admission_refs)}
    window_id = 'observation-window:' + _digest(payload)[:24]
    return ObservationWindow(window_id=window_id, reference_now_ns=reference_now_ns, max_join_skew_ns=max_join_skew_ns, max_clock_uncertainty_ns=max_clock_uncertainty_ns, current_ref_ids=tuple((item.ref_id for item in current)), stale_ref_ids=tuple((item.ref_id for item in stale)), unaligned_ref_ids=tuple((item.ref_id for item in unaligned)), source_refs=tuple(source_refs), alignment_witness_refs=tuple(witness_refs), alignment_admission_refs=tuple(admission_refs), provenance_refs=tuple(sorted(provenance)))
__all__ = ['CLOCK_ALIGNMENT_WITNESS_SCHEMA', 'OBSERVATION_WINDOW_SCHEMA', 'TEMPORAL_REF_SCHEMA', 'ClockAlignmentWitness', 'ObservationWindow', 'PerceptionTemporalError', 'TemporalPerceptRef', 'bind_observed_claim', 'build_observation_window']
