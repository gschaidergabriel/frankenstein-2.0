...
def _validate_result_against_fixture(fixture: MicroWorldFixture, result: OrientationRunResult) -> str:
    """Replay current result content at the comparison trust boundary.

    Frozen dataclasses are not a security boundary: ``object.__setattr__`` can mutate a
    previously valid object. Therefore comparison must not trust cached result fields merely
    because construction once succeeded. Replaying the public trace against evaluator-only
    fixture state re-binds score, terminal state, budget state and final observation to the
    exact current content before any comparison delta is computed.
    """
    ...
    if result.schema != RESULT_SCHEMA or result.classification != RESULT_CLASSIFICATION:
        raise OrientationBenchmarkError("result schema/classification mismatch at comparison boundary")
    ...
    if type(result.run_descriptor) is not RunDescriptor:
        raise OrientationBenchmarkError("result run_descriptor must be exact concrete RunDescriptor")
    try:
        result.run_descriptor.assert_matches_fixture(fixture)
    except CognitiveMicroWorldError as exc:
        raise OrientationBenchmarkError("result run descriptor no longer matches exact fixture/provenance") from exc
    if type(result.public_trace) is not tuple or any(type(item) is not PublicTraceEntry for item in result.public_trace):
        raise OrientationBenchmarkError("result public_trace must contain exact PublicTraceEntry values")
    if len(result.public_trace) > fixture.max_steps:
        raise OrientationBenchmarkError("result public trace exceeds fixture step budget")
    ...
    for entry in result.public_trace:
        ...
        if entry.schema != TRACE_ENTRY_SCHEMA:
            raise OrientationBenchmarkError("result trace-entry schema mismatch")
        if entry.step_index != state.step_index:
            raise OrientationBenchmarkError("result trace step index does not replay")
        if entry.observation_view_sha256 != observation.sha256():
            raise OrientationBenchmarkError("result trace observation-view digest does not replay")
        if entry.observation_ref != observation.observation_ref:
            raise OrientationBenchmarkError("result trace observation reference does not replay")
        if entry.observation_payload_sha256 != observation.observation_sha256:
            raise OrientationBenchmarkError("result trace observation payload digest does not replay")
        try:
            request = ActionRequest.for_observation(observation, action_id=entry.action_id)
            state, observation, _ = step_episode(fixture, state=state, request=request)
        except CognitiveMicroWorldError as exc:
            raise OrientationBenchmarkError("result trace action does not replay against exact fixture") from exc

    replay_terminal = observation.terminal
    replay_exhausted = not replay_terminal and state.step_index >= fixture.max_steps
    if type(result.terminal_reached) is not bool or result.terminal_reached != replay_terminal:
        raise OrientationBenchmarkError("result terminal state does not match deterministic replay")
    if type(result.step_budget_exhausted) is not bool or result.step_budget_exhausted != replay_exhausted:
        raise OrientationBenchmarkError("result step-budget state does not match deterministic replay")
    if type(result.steps_used) is not int or result.steps_used != len(result.public_trace):
        raise OrientationBenchmarkError("result steps_used does not match replayed public trace")
    if type(result.evaluator_score) is not int or result.evaluator_score != state.cumulative_score:
        raise OrientationBenchmarkError("result evaluator score does not match deterministic replay")
    if result.final_observation_view_sha256 != observation.sha256():
        raise OrientationBenchmarkError("result final observation digest does not match deterministic replay")
    ...
    payload = {
        ...
        "episode_id": result.episode_id,
        "episode_generation": result.episode_generation,
        "public_trace_sha256": _digest([entry.as_dict() for entry in result.public_trace]),
        ...
        "final_observation_view_sha256": result.final_observation_view_sha256,
    }
    expected_result_id = "orientation-result:" + _digest(payload)
    if result.result_id != expected_result_id:
        raise OrientationBenchmarkError("result_id does not bind exact current replayed result content")
...
def compare_orientation_runs(
    ...
    *,
    ...
    baseline_sha256 = _validate_result_against_fixture(fixture, baseline)
    intervention_sha256 = _validate_result_against_fixture(fixture, intervention)
    ...
    payload = {
        ...
        "intervention_result_sha256": intervention_sha256,
    }
