...
| `host_identity_sha256` real scheme | design | **Vorschlag existiert (2026-09-03), wartet auf Owner-Wahl** | Every run through F-ITER3/local-iter2 used `/etc/machine-id` as an explicitly test-scoped stand-in; no real scheme had ever been proposed. This round (paket-1788424194135-8f29c3) is the first to actually design one: 3 candidate schemes (A: salted local `/etc/machine-id` hash — pepper stored in v1 `unified.db`/Tresor, never leaves host; B: fresh random token generated once and stored in `unified.db`, independent of any OS/hardware id; C: multi-factor OS/hardware composite — flagged as NOT recommended, bigger PII/leak surface, less stable). Recommends **A**, because `state_migration.py`'s own invariant (`target.host_identity_sha256 == source.root.host_identity_sha256`, i.e. one migration = one host) reads literally as an OS/machine-level identity, which A matches and B/C don't (B tracks DB-lineage continuity, a different and already-separately-covered concept via `lineage_id`/`state_sha256`). No code activates any candidate — reference sketch only, unaktiviert. | Gabriel decision among A/B/C (or a 4th) needed before any `StateLineage` work can start. Full writeup: `workpackages/evidence_inbox/F2-WP-1207/self_integration/HOST_IDENTITY_PROPOSAL_20260903.md`, `gschaidergabriel/self-integration` `log/2026-09-03-013-host-identity-proposal.md`. |
...
2. **A real `host_identity_sha256` scheme** — **Vorschlag existiert, wartet auf
   Owner-Wahl (2026-09-03).** Before this round only a test-scoped `/etc/machine-id`
   stand-in existed anywhere in either codebase, never a real proposal. See the Part
   4 table row above for the 3 candidates and the recommendation; still zero code
   activates any of them — the owner has not chosen, and no `StateLineage` object
   has ever been constructed with a real (non-placeholder) value.
