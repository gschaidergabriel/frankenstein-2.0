...
| GRID10 / `StateRootIdentity` wiring into v1 | design | **Vorschlag existiert, PLUS End-zu-End-SHADOW-Demonstration gegen echte historische Turn-Daten UND gegen v1s echte SHADOW-Retrievalfunktion (2026-09-03), weiterhin keine Live-Aktivierung** | Package `paket-1788388214637-d765a9` built an opt-in, default-OFF `f2wp1207-wiring-diagnose` subcommand in a **fresh clone** of `gschaidergabriel/frankenstein` (never `~/frankenstein-repo`) — activation only via `STERN_F2WP1207_WIRING=1` env var, no-op otherwise; 0-delta proven via `--help` diff. **2026-09-03 follow-up (`paket-1788426634287-6f53f2`):** a full pipeline `UserPromptSubmit → TypedEntry → StateRootIdentity → GRID10 frame (SHADOW) → [v1-processing placeholder, NOT executed] → Output → persisted minimal reentry evidence` was built and run, in a **fresh clone** of `gschaidergabriel/frankenstein-2.0`, against **8 real, already-completed turn-cycle markers pulled read-only from the real `unified.db`** (`turns` LEFT JOIN `retrieval_episodes`, session `a2f7b438-...`, the current live session) instead of the prior round's synthetic ten-cell exercise. Uses `frankenstein2.entity_identity.StateRootIdentity` (the variant WITH `installation_id`, from the `entity-identity-layering-v2` branch) and the unmodified `frankenstein2.grid10_interface` ABI. Result: 8/8 turns processed, 10/10 GRID10 cells completed every run, 8/8 distinct record/plan/typed-entry SHA-256 (non-uniform, real-data-driven), total work-units-used spread 27–41 across turns. Step 5 ("bestehende v1-Verarbeitung") is an explicit labeled placeholder, never executed. Still: no v1 concept mapped to any GRID10 cell (deliberately functionally neutral, same discipline as the `grid10_observation_schema.py` round); `EntityIdentity`/`installation_id` demo-minted, not canonical; nothing wired into the real `stern.py` hook chain. **2026-09-03 Schritt 3-5 follow-up (`paket-1788441744564-511a19`):** the Step-4 placeholder was replaced with a call to v1's REAL, UNMODIFIED `stern.automatischer_abruf()` — the exact function v1's live hook chain already calls every turn (writes `star_abruf_gezeigt` rows as part of its normal, unmodified behavior) — but with its module-global `DB_PATH` monkeypatched to a fully isolated, freshly-seeded throwaway SQLite DB, never the real `unified.db`. Zero-delta proof over 5 synthetic prompts, pipeline-toggle OFF vs ON: OFF leaves the isolated DB byte-identical (`db_unchanged: true`); ON produces genuinely distinguishing, honest output (real hits for two on-topic prompts, correct "kein Treffer" for two unrelated ones, and one honest documented false-positive from a bare substring match rather than a hidden one); TypedEntry/GRID10-frame SHA-256 identical OFF vs ON for the same prompt, proving only the step-4 output differs. `~/frankenstein-repo` opened zero times in the new file (the real `stern.py` source came from a separate, already-existing fresh `/tmp/fork-v1` clone); the real `unified.db` never opened at all. | coordinator/owner design decision on whether/how v1 should adopt either module, PLUS an explicit owner sign-off before any real activation against `~/frankenstein-repo`/`unified.db`. Branches: `self-integration/wp1207-v1-wiring-proposal-20260903T053400Z` (commit `917cb9e`, `gschaidergabriel/frankenstein`) for the diagnostic-subcommand proposal; `self-integration/wp1207-entity-identity-layering-v2-20260903` (`gschaidergabriel/frankenstein-2.0`) for the pipeline demo + Schritt-3-5 shadow-hook-wiring, both pushed, neither merged. Details: `gschaidergabriel/self-integration` `log/2026-09-03-012-v1-wiring-proposal.md`, the pipeline-demo log entry, and the new Schritt-3-5 log entry. **Explicitly NOT part of this round:** actual registration in `~/frankenstein-repo`'s live `hooks.json`/`stern.py` dispatch chain — that is a separate, higher-authorization step (blast radius = every session on the machine), deliberately excluded by the coordinator regardless of the general go-ahead given for this round. |
...
4. **No wiring of any v2 self-integration primitive into v1's actual `stern.py`
   ...
   **2026-09-03 follow-up (`SHADOW-PIPELINE-DEMO`):** the full
   ...
   `~/frankenstein-repo` or a live process. **2026-09-03 further follow-up
   (Schritt 3-5):** step 5 is no longer just a placeholder — it now calls v1's
   real, unmodified `automatischer_abruf()` SHADOW-retrieval function, proven
   with a 0-delta pipeline-toggle test, but still only against an isolated
   throwaway DB, never the real `unified.db`. Neither this nor the
   `f2wp1207-wiring-diagnose` proposal has ever been activated against a real
   running v1 instance / the real, registered `~/frankenstein-repo` hook chain;
   that remains entirely an owner decision.
