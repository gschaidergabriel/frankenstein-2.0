...
def test_software_loopback_rejects_late_old_generation_and_never_commits_sentinel():
    ...
    # Fail closed on all scopes this reference sink cannot observe.
    for key in (
        "physical_speaker",
        "physical_microphone",
        "human_heard_output",
...
def test_pulse_mode_is_explicitly_virtual_not_physical_credit():
    # This test does not require pactl/pacat/parec. It locks the evidence law
    # into the source path so later VPS execution cannot silently inflate scope.
