...
If migration succeeds, the caller can self-attest the row being migrated and the
...
from frankenstein2.persistent_agency_authority_migration import migrate_legacy_authority_receipt
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            assert receipt.evidence_refs == ("review:caller-self-attested-not-admitted",)
            ...
            print(
                "PASS_REPRODUCED_NEGATIVE_CONTROL: arbitrary row-derived legacy receipt "
                "migrated without authenticated external admission evidence"
            )
            return 0
        finally:
            store.close()
