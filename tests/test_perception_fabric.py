...
def test_source_binds_single_capture_owner_identity():
    ...
    assert source.capture_owner_id == "capture-owner:camera:front"
    assert source.as_dict()["world_truth_authority"] == "NONE"


def test_analyze_requires_see():
    with pytest.raises(PerceptionFabricError, match="ANALYZE requires SEE"):
        snapshot(capabilities=(PerceptionCapability.ANALYZE,))


def test_external_vlm_capability_requires_analyze():
    with pytest.raises(PerceptionFabricError, match="EXTERNAL_VLM requires ANALYZE"):
        snapshot(capabilities=(PerceptionCapability.SEE, PerceptionCapability.EXTERNAL_VLM))


def test_stale_permission_digest_fails_closed():
    s1 = snapshot(generation=1)
    i = intent(s1)
    s2 = snapshot(generation=2)
    with pytest.raises(PerceptionFabricError, match="stale or mismatched"):
        i.validate_against(s2, now_monotonic_ns=100)


def test_revoked_source_permission_fails_closed():
    allowed = snapshot()
    i = intent(allowed)
    revoked = snapshot(capabilities=(), generation=1)
    with pytest.raises(PerceptionFabricError):
        i.validate_against(revoked, now_monotonic_ns=100)


def test_expired_snapshot_and_expired_intent_fail_closed():
    s = snapshot(expires=200)
    i = intent(s, expires=150)
    with pytest.raises(PerceptionFabricError, match="permission snapshot"):
        i.validate_against(s, now_monotonic_ns=250)

    fresh = snapshot(expires=1_000)
    expired_intent = intent(fresh, expires=150)
    with pytest.raises(PerceptionFabricError, match="ObserveIntent is expired"):
        expired_intent.validate_against(fresh, now_monotonic_ns=150)


def test_remote_frame_requires_explicit_capability():
    s = snapshot()
    i = intent(s, remote=True)
    with pytest.raises(PerceptionFabricError, match="REMOTE_FRAME"):
        i.validate_against(s, now_monotonic_ns=100)


def test_external_vlm_requires_explicit_capability():
    ...
    with pytest.raises(PerceptionFabricError, match="EXTERNAL_VLM"):
        i.validate_against(s, now_monotonic_ns=100)


def test_zero_workers_is_valid_and_defers_all_work():
    s = snapshot()
    ...
    assert result.selected_intent_ids == ()
    assert set(result.deferred_intent_ids) == {"a", "b"}
    assert result.total_work_units == 0


def test_scheduler_never_selects_more_than_four_and_prefers_priority():
    ...
    assert len(result.selected_intent_ids) == 4
    assert set(result.selected_intent_ids) == {"i2", "i3", "i4", "i5"}
    assert set(result.deferred_intent_ids) == {"i0", "i1"}


def test_worker_budget_is_hard_ceiling():
    ...
    assert result.selected_intent_ids == ("a",)
    assert result.deferred_intent_ids == ("b",)
    assert result.total_work_units == 7


def test_duplicate_permission_snapshot_for_same_source_fails_closed():
    s1 = snapshot(generation=1)
    s2 = snapshot(generation=2)
    i = intent(s1)
    with pytest.raises(PerceptionFabricError, match="exactly one permission snapshot"):
        allocate_perception_workers(
            intents=(i,),
            policy=policy(),
            permission_snapshots=(s1, s2),
...
def test_source_count_is_independent_of_worker_count():
    ...
    assert len(result.selected_intent_ids) == 3
    assert len(result.deferred_intent_ids) == 6


def test_baseline_intent_has_no_vlm_or_remote_payload_authority():
    s = snapshot()
    i = intent(s)
    i.validate_against(s, now_monotonic_ns=100)
    payload = i.as_dict()
    assert payload["allow_remote_frame"] is False
    assert payload["allow_external_vlm"] is False
    assert payload["perception_execution_authority"] == "NONE"
