"""Canonical regression for the F2-WP-506 G3 nested SelectedCandidate trust boundary.

Reproduces REVIEW_ONLY PR #274 at repository-component scope. A SelectedCandidate
subtype must not be able to self-report canonical serialized lineage while exposing
different direct attributes later consumed by create_broadcast().
"""
...
def _plan() -> Grid10Plan:
    ...
    return Grid10Plan.create(
        plan_id="grid-plan-selectedcandidate-regression",
        cycle_id="cycle-selectedcandidate-regression",
        generation=3,
        frame_id="frame-selectedcandidate-regression",
        frame_generation=4,
        frame_sha256=D,
        policy_id="grid-policy-selectedcandidate-regression",
        policy_generation=1,
        policy_sha256=G,
        cells=cells,
        max_total_work_units=80,
        provenance_refs=("prov:grid-plan-selectedcandidate-regression",),
    )
...
def _selection() -> WorkspaceSelection:
    ...
    policy = SelectionPolicy(
        policy_id="gwt-policy-selectedcandidate-regression",
        generation=1,
    ...
    return build_workspace_selection(
        selection_id="selection-selectedcandidate-regression",
...
class ForgedSelectedCandidate(SelectedCandidate):
    """Expose a forged direct payload while serializing the canonical payload."""
...
def _forged_item(canonical: WorkspaceSelection) -> ForgedSelectedCandidate:
    original = canonical.selected[0]
    return ForgedSelectedCandidate(
...
def _selection_kwargs(canonical: WorkspaceSelection, selected):
    return dict(
...
def test_wp506_constructor_rejects_selected_candidate_subtype():
    canonical = _selection()
    forged_item = _forged_item(canonical)

    with pytest.raises(GwtWorkspaceError, match="concrete SelectedCandidate"):
        WorkspaceSelection(**_selection_kwargs(canonical, (forged_item,)))


def test_wp506_consumption_boundary_rejects_low_level_selected_member_tamper():
    canonical = _selection()
    forged_item = _forged_item(canonical)

    # Simulate state that bypassed dataclass construction through a low-level mutation.
    # The public consumption boundary must independently reject this before trusting
    # member serialization or direct payload attributes.
    forged = WorkspaceSelection(**_selection_kwargs(canonical, canonical.selected))
    object.__setattr__(forged, "selected", (forged_item,))

    # The old exploit property is preserved: the polymorphic member serialization can
    # still make the mutated selection self-report the canonical digest.
    assert forged.sha256() == canonical.sha256()

    with pytest.raises(GwtWorkspaceError, match="concrete SelectedCandidate"):
        create_broadcast(
            broadcast_id="broadcast-selectedcandidate-regression",
            generation=1,
            selection=forged,
