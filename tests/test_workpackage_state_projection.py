...
# This is the exact stale set admitted by F2-WP-002 generation 5. The test
# intentionally does not claim authority over unrelated workpackages.
G5_RECONCILED_IDS = (
    "F2-WP-200",
    "F2-WP-201",
    "F2-WP-203",
...
class WorkpackageStateProjectionTests(unittest.TestCase):
    ...
    def test_accepted_pointer_cannot_project_below_accepted_at_scope(self) -> None:
        workpackages = _aggregate_state()["workpackages"]
        for workpackage_id in G5_RECONCILED_IDS:
    ...
    def test_accepted_projection_has_granular_evidence_reference(self) -> None:
        workpackages = _aggregate_state()["workpackages"]
        for workpackage_id in G5_RECONCILED_IDS:
            claim = _active_claim(workpackage_id)
