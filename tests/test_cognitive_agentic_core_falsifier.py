...
class AgenticCoreFalsifierTests(unittest.TestCase):
    ...
    def test_action_budget_is_policy_bound_and_fail_closed(self):
        high_action = tuple(
            evidence(capability, actions=200_000_000)
            for capability in (EXPLORATION, MODELING, GOAL_SETTING, PLANNING_EXECUTION)
        )
        report = evaluate_agentic_core(high_action, policy=policy(), report_id="report-action-budget")
        self.assertEqual(report.verdict, FALSIFIED)
        self.assertEqual(report.total_action_count, 800_000_000)
        for capability in (EXPLORATION, MODELING, GOAL_SETTING, PLANNING_EXECUTION):
            self.assertIn(f"ACTION_BUDGET_EXCEEDED:{capability}", report.reasons)

    def test_action_budget_boundary_is_inclusive(self):
        values = tuple(
            evidence(capability, actions=1_000)
            for capability in (EXPLORATION, MODELING, GOAL_SETTING, PLANNING_EXECUTION)
        )
        report = evaluate_agentic_core(values, policy=policy(), report_id="report-action-boundary")
        self.assertEqual(report.verdict, SUPPORTED_AT_COMPONENT_SCOPE)
        self.assertNotIn("ACTION_BUDGET_EXCEEDED", "|".join(report.reasons))
