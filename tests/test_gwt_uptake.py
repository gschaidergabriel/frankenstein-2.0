...
def broadcast(recipients=GRID10_CELL_IDS, bid="b1"):
    return BroadcastEnvelope(
        ...
        plan_generation=4,
        plan_sha256=B,
        recipient_cell_ids=tuple(recipients),
        candidate_ids=("c1",),
...
def test_receipt_binds_full_broadcast_selection_plan_identity():
    b = broadcast(("G1", "G10"))
    observed = receipt(b, "G1", "UPTAKEN")
    assert observed.broadcast_sha256 == b.sha256()
    assert observed.cycle_id == b.cycle_id
    assert observed.broadcast_generation == b.generation
    assert observed.selection_generation == b.selection_generation
    assert observed.selection_sha256 == b.selection_sha256
    assert observed.plan_generation == b.plan_generation
    assert observed.plan_sha256 == b.plan_sha256


def test_subset_recipient_complete_is_complete_not_unknown():
    b = broadcast(("G1",))
    observed = summary(b, "G1")
    assert observed.unknown_cell_ids == ()
    assert observed.uptaken_cell_ids == ("G1",)
    assert observed.status == "UPTAKE_OBSERVED"


def test_subset_recipient_missing_actual_recipient_is_unknown():
    b = broadcast(("G1", "G10"))
    observed = summarize_uptake(
        summary_id="s",
        broadcast=b,
        receipts=(receipt(b, "G1"),),
        provenance_refs=("p",),
    )
    assert observed.unknown_cell_ids == ("G10",)
    assert observed.status == "UNKNOWN_INCOMPLETE_RECEIPTS"


def test_nonrecipient_receipt_fails_closed():
    with pytest.raises(GWTUptakeError, match="recipient"):
        receipt(broadcast(("G1",)), "G2")


def test_duplicate_cell_receipt_fails_closed():
    b = broadcast(("G1",))
    first = receipt(b, "G1")
    with pytest.raises(GWTUptakeError, match="duplicate logical"):
        summarize_uptake(
            summary_id="s",
            broadcast=b,
...
def test_direct_receipt_constructor_cannot_be_admitted():
    b = broadcast(("G1",))
    forged = replace(receipt(b, "G1"), _factory_seal=None)
    with pytest.raises(GWTUptakeError, match="observation factory"):
        summarize_uptake(
            summary_id="s",
            broadcast=b,
            receipts=(forged,),
...
def test_stale_broadcast_binding_fails_closed():
    b = broadcast(("G1",))
...
def test_delivery_without_uptake_is_not_uptake():
    b = broadcast(("G1", "G2"))
    observed = summary(b)
    assert observed.status == "NO_UPTAKE_OBSERVED"
    assert observed.delivered_cell_ids == ("G1", "G2")
    assert observed.uptaken_cell_ids == ()


def test_not_observed_delivery_forces_unknown():
    b = broadcast(("G1",))
    with pytest.raises(GWTUptakeError, match="must remain UNKNOWN"):
        receipt(b, "G1", "NOT_UPTAKEN", "NOT_OBSERVED")


def test_non_uptaken_cannot_carry_downstream_evidence():
    b = broadcast(("G1",))
    with pytest.raises(GWTUptakeError, match="must not carry downstream"):
...
def test_positive_causal_result_requires_complete_uptake_and_matched_probe():
    b = broadcast(("G1", "G10"))
    ...
    result = evaluate_causal_influence(
        result_id="res",
        broadcast=b,
        uptake_summary=observed,
        intervention=intervention,
        control=control,
        provenance_refs=("p",),
    )
    assert result.status == "CAUSAL_INFLUENCE_OBSERVED_AT_CONTRACT_SCOPE"
    assert result.as_dict()["runtime_credit"] == 0


def test_unmatched_nonbroadcast_input_is_unknown():
    b = broadcast(("G1",))
    observed = summary(b, "G1")
...
def test_same_downstream_output_is_no_observed_influence():
    b = broadcast(("G1",))
    observed = summary(b, "G1")
...
def test_different_probe_ids_are_unmatched():
    b = broadcast(("G1",))
    observed = summary(b, "G1")
...
def test_incomplete_uptake_blocks_causal_positive():
    b = broadcast(("G1", "G2"))
    observed = summarize_uptake(
        summary_id="s",
        broadcast=b,
...
def test_direct_positive_summary_constructor_bypass_rejected():
    b = broadcast(("G1",))
    forged = replace(summary(b, "G1"), _factory_seal=None)
    intervention, control = arms(b)
    with pytest.raises(GWTUptakeError, match="summarizer"):
        evaluate_causal_influence(
            result_id="r",
            broadcast=b,
...
def test_summary_source_receipt_tamper_rejected():
    b = broadcast(("G1",))
    real = summary(b, "G1")
    forged_receipt = replace(
        real.source_receipts[0],
        uptake_status="NOT_UPTAKEN",
        downstream_ref=None,
    ...
    intervention, control = arms(b)
    with pytest.raises(GWTUptakeError, match="lineage mismatch"):
        evaluate_causal_influence(
            result_id="r",
            broadcast=b,
            uptake_summary=forged,
...
def test_direct_probe_constructor_bypass_rejected():
    b = broadcast(("G1",))
    observed = summary(b, "G1")
    intervention, control = arms(b)
    forged = replace(intervention, _factory_seal=None)
    with pytest.raises(GWTUptakeError, match="arm factory"):
        evaluate_causal_influence(
            result_id="r",
            broadcast=b,
            uptake_summary=observed,
