"""Canonical WP506 G3 regression for PR #274 nested SelectedCandidate subtype lineage split.

Repository component regression only. This test does not establish target runtime, physical
GRID10 execution, GWT uptake/causal influence, J-Space semantics, effects, training, or
...
def _selection() -> WorkspaceSelection:
    ...
    plan = Grid10Plan.create(
        plan_id="grid-plan-g3-nested-regression",
        cycle_id="cycle-g3-nested-regression",
        generation=3,
        frame_id="frame-g3-nested-regression",
        frame_generation=4,
        frame_sha256=D,
        policy_id="grid-policy-g3-nested-regression",
        policy_generation=1,
        policy_sha256=G,
        cells=cells,
        max_total_work_units=80,
        provenance_refs=("prov:grid-plan-g3-nested-regression",),
    )
    ...
    policy = SelectionPolicy(
        policy_id="gwt-policy-g3-nested-regression",
        generation=1,
    ...
    return build_workspace_selection(
        selection_id="sel-g3-nested-regression",
...
class ForgedSelectedCandidate(SelectedCandidate):
    """Expose forged direct state while reporting canonical serialized lineage."""
