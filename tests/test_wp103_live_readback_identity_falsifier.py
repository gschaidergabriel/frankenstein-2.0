...
class WP103LiveReadbackIdentityFalsifier(unittest.TestCase):
    def test_readback_fails_closed_after_canonical_db_inode_replacement(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store = CanonicalDeliveryStore.open(
            ...
            identity = CausalIdentity(
                session_id="session:readback-falsifier",
                agent_id="agent:review-only",
                task_id="task:wp103-live-readback",
                turn_id="turn:1",
                causal_id="causal:wp103-live-readback",
                generation=1,
            )
            ...
            # Canonical authority moved to a different inode. A long-lived store must not
            # report state from the now-unlinked historical inode as current readback.
            with self.assertRaisesRegex(
                DeliveryStoreError,
                "UNIFIEDDB_LIVE_FILE_IDENTITY_DRIFT",
            ):
                store.get_delivery(
                    causal_event_id=identity.causal_id,
                    recipient_id="recipient:alpha",
