"""WP206 G8 regression: preserve WP900 G5 runtime identity through canonical persistence.

Repository-only discriminator. It proves that two valid runtime subjects may share one
deterministic WholePersistentLoopSeal while retaining distinct runtime/source identity in
the evidence returned by the existing WP206 persistence authority.
"""
...
def test_runtime_bound_persistence_preserves_distinct_valid_g5_subjects(tmp_path):
    ...
    assert payload["repository_component_credit"] == 0
