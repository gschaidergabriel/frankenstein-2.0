"""WP206 G8 repository discriminator for runtime-identity persistence."""
...
def test_runtime_bound_persistence_preserves_distinct_valid_g5_subjects(tmp_path):
    ...
    try:
        ...
        evidence_a = persist_runtime_bound_successor_and_readback(
            store, seal=whole_a, runtime_binding=candidate_a, next_checkpoint=successor
        )
        evidence_b = persist_runtime_bound_successor_and_readback(
            store, seal=whole_b, runtime_binding=candidate_b, next_checkpoint=successor
        )
    finally:
        store.close()
...
def test_forged_runtime_identity_fails_closed_before_successor_write(tmp_path):
    ...
    try:
        ...
        with pytest.raises(
            ...
            persist_runtime_bound_successor_and_readback(
                store, seal=whole, runtime_binding=forged, next_checkpoint=successor
            )
