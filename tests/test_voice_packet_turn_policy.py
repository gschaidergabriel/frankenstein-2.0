...
class VoicePacketTurnPolicyTests(unittest.TestCase):
    ...
    def test_pfd7_stale_hold_event_cannot_emit_policy_after_later_final_endpoint(self) -> None:
        cortex = VoicePacketCortex(self.session())
        accepted_hold = cortex.accept_input(self.hold_packet(cortex))
        final = VoiceInputPacket(
            session_id=cortex.session_id,
            turn_id="turn-1",
        ...
        with self.assertRaises(PacketTurnPolicyError):
            apply_packet_turn_policy(cortex, accepted_hold, self.policy("BACKCHANNEL"), monotonic_ms=130)
        self.assertEqual(tuple(cortex.events), before)
        self.assertFalse(
            any(
                event.event_kind == "VOICE_INTENT"
                and f"source_event_id={accepted_hold.event_id}" in event.detail
