...
class WP711UnadmittedClockWitnessFalsifier(unittest.TestCase):
    def test_matching_witness_must_not_be_its_own_admission_authority(self):
        """A caller-created witness is evidence input, not admission authority.

        This is deliberately review-only counterevidence.  The current G2 API accepts
        ``alignment_witnesses`` directly.  If merely supplying a syntactically valid witness
        makes two otherwise-unrelated clock domains ALIGNED, the witness can self-authorize
        the exact mapping it asserts.  The fail-closed invariant is that cross-clock
        comparison remains UNALIGNED until an independent admission boundary binds the exact
        witness identity (for example an admitted witness digest or canonical registry ref).
        """
