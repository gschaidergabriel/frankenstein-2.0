...
def test_valid_wp105_target_cannot_enter_wp900_as_verified_applied():
    ...
    # This is the only valid construction state for the existing WP105 target.
    ...
    # WP900's existing verified outcome consumer requires this same concrete target
    # type to already be VERIFIED_APPLIED. The two current contracts therefore cannot
    # compose at the verified-applied boundary without an authoritative contract fix.
    with pytest.raises(
        WholePersistentLoopError,
