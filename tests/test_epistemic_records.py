...
class EpistemicRecordTests(unittest.TestCase):
    ...
    def test_payload_mapping_keys_must_be_strings(self):
        # JSON object keys are strings. Silently coercing an integer key would collapse
        # distinct caller inputs such as {1: "x"} and {"1": "x"} onto one identity.
    ...
    def test_identity_and_reference_text_must_be_canonical(self):
        # Identity/provenance references must not admit invisible whitespace/control aliases.
        with self.assertRaises(EpistemicRecordError):
            ObservedEvidence.create(
                record_id=" obs-space ",
                generation=1,
        ...
        with self.assertRaises(EpistemicRecordError):
            ObservedEvidence.create(
                record_id="obs-control",
                generation=1,
                payload={"value": 1},
                provenance_sha256=PROVENANCE_A,
                observation_ref="sensor:\n1",
            )
