#!/usr/bin/env python3
"""F2-WP-206 G2 executable regressions for UnifiedDB authority-receipt replay binding."""
...
class WP206UnifiedDBAuthorityReceiptReplayFence(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _replace_stored_receipt(self, value: str) -> None:
        ...
        try:
            ...
            self.assertEqual(after[0], value)
        finally:
            connection.close()

    def _assert_replay_rejects_receipt(self, value: str) -> None:
        self._write_checkpoint()
        self._replace_stored_receipt(value)
        reader = self._probe("read")
        self.assertNotEqual(reader.returncode, 0, msg=reader.stdout)
    ...
    def test_different_well_formed_receipt_fails_closed(self) -> None:
        self._assert_replay_rejects_receipt("0" * 64)

    def test_malformed_receipt_fails_closed(self) -> None:
        self._assert_replay_rejects_receipt("not-a-sha256-receipt")

    def test_empty_receipt_fails_closed(self) -> None:
        self._assert_replay_rejects_receipt("")
