...
class TargetCompletionEpistemicsTests(unittest.TestCase):
    def test_missing_mandatory_evidence_is_unknown_not_pass(self) -> None:
        ...
        report = TargetCompletionReport(target_id="host-a", evaluated_fidelity=FidelityLevel.T1_UBUNTU_USERSPACE, obligations=(item,))
    ...
    def test_target_mismatch_and_duplicate_ids_fail_closed(self) -> None:
        ...
        with self.assertRaises(CompletionEpistemicsError):
            TargetCompletionReport(target_id="host-a", evaluated_fidelity=FidelityLevel.T1_UBUNTU_USERSPACE, obligations=(good, wrong_target))
        with self.assertRaises(CompletionEpistemicsError):
            TargetCompletionReport(target_id="host-a", evaluated_fidelity=FidelityLevel.T1_UBUNTU_USERSPACE, obligations=(good, good))

    def test_empty_report_is_rejected_not_vacuously_complete(self) -> None:
        with self.assertRaises(CompletionEpistemicsError):
            TargetCompletionReport(target_id="host-a", evaluated_fidelity=FidelityLevel.T0_CONTRACT, obligations=())
