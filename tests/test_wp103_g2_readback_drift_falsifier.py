...
class WP103G2ReadbackDriftFalsifier(unittest.TestCase):
    def _open_populated_store(self, tmp: str):
        ...
        identity = CausalIdentity(
            session_id="session:wp103-g2-readback-drift",
            agent_id="agent:candidate-falsifier",
            task_id="task:wp103-g2-readback-drift",
            turn_id="turn:1",
            causal_id="causal:wp103-g2-readback-drift",
            generation=1,
        )
    ...
    def test_get_delivery_fails_closed_after_canonical_inode_replacement(self):
    ...
    def test_transition_count_fails_closed_after_canonical_inode_replacement(self):
        with tempfile.TemporaryDirectory() as tmp:
