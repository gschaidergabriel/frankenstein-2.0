#!/usr/bin/env python3
"""Review-only executable falsifier for F2-WP-206 DB-authority receipt replay binding.

This file intentionally does not modify WP-206 production source or claim canonical
mutation authority.  It turns the source-level candidate falsifier recorded on main
into one positive control plus one fail-closed regression requirement.
"""
...
class WP206DBAuthorityReceiptReplayFalsifier(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_tampered_stored_authority_receipt_must_fail_closed(self) -> None:
        self._write_checkpoint()
        forged_receipt = "0" * 64
        connection = sqlite3.connect(self.db)
        try:
        ...
        self.assertNotEqual(
            ...
            0,
            msg=(
                "FALSIFIER REPRODUCED: replay succeeded after tampering only the "
                "stored unifieddb_authority_receipt_sha256 column. WP-206 must "
                "fail closed on an authority-receipt mismatch. stdout="
                + reader.stdout
            ),
        )
