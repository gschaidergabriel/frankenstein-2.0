...
class Trigger4G2RuntimeLauncherContractTests(unittest.TestCase):
    ...
    def test_live_bound_and_terminal_analyzer_are_wired(self) -> None:
        launcher = LAUNCHER.read_text(encoding="utf-8")
        ...
        self.assertIn('G2_HARNESS_ANALYZER="$PWD/research/local_voice/tools/t7_pipewire_g2_h4_guard.py"', launcher)
    ...
    def test_harness_exit_is_not_erased_by_launcher(self) -> None:
        ...
        tail = launcher.split('printf "G2_PIPEWIRE_HARNESS_EXIT=%s\\n" "$harness_status"', 1)[1]
