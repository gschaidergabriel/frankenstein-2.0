        ...
        *,
        ...
        return LegacyAuthorityMigrationManifest.create(
            ...
            provenance_refs=(
                ...
                "review-pr663:accepted-g1-upgrade-compat-falsifier",
                "supervisor:9.13bh-migration-compatibility-gate",
            ),
        )

    def test_exact_g1_receipt_is_rejected_then_explicitly_migrated_and_tamper_stays_red(self) -> None:
        writer = self._probe("write")
        self.assertEqual(writer.returncode, 0, msg=writer.stderr)
        writer_payload = json.loads(writer.stdout)

        # The restart probe deliberately exposes the historical G1 receipt family:
        # UnifiedDBFingerprint.receipt_sha256().  Bind that exact observed value rather
        # than substituting a shape-compatible synthetic 64-hex placeholder.
        legacy_receipt = str(writer_payload["db_authority_receipt"])
        ...
        try:
            self.assertNotEqual(
                ...
                "the exact G1 receipt and current bound-file receipt must be distinct for this discriminator",
            )
