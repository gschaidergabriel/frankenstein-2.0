...
Review-only PR #746 proved that treating connection-local PRAGMA main.data_version as the
verdict is database-wide: a valid WP103 typed commit changed data_version and incorrectly
invalidated unchanged WP206 state. G6 therefore uses data_version only as a dirty hint and a
bounded in-memory witness of the WP206-owned checkpoint schema and rows as the verdict.

Repository-component evidence only; no target/runtime/GRID10/GWT/J-Space/effect/completion/
training/whole-system credit is granted by this test.
"""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            try:
                ...
                raise AssertionError("external WP206-owned schema mutation did not fail closed")
        finally:
            agency.close()
