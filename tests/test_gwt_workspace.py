...
class ForgedSelectedCandidate(SelectedCandidate):
    """Expose forged attributes while serializing a canonical payload lineage."""

    def as_dict(self):
        payload = super().as_dict()
        payload["payload_ref"] = "payload:nested-canonical"
        return payload
...
def test_workspace_selection_rejects_selected_candidate_subtype_attribute_lineage_split():
    canonical = selection((candidate("nested-canonical", payload_ref="payload:nested-canonical"),))
    original = canonical.selected[0]
    forged = ForgedSelectedCandidate(
        candidate_id=original.candidate_id,
        candidate_sha256=original.candidate_sha256,
        payload_ref="payload:nested-forged",
    ...
    assert forged.as_dict()["payload_ref"] == "payload:nested-canonical"
    assert forged.payload_ref == "payload:nested-forged"
