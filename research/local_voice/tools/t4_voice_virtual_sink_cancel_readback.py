...
class PulseNullSink:
    ...
    def write(self, pcm: bytes) -> None:
        ...
        # Pace approximately at real time so cancellation can race queued audio.
    ...
    def reset(self) -> None:
        # A fresh pacat process is the new playback generation.
        self.player = None
...
def _count_exact_chunk(blob: bytes, pattern: bytes) -> int:
    # Exact byte matching is valid for the software reference.  Pulse null-sink
    # may add alignment/latency, so the runtime result also records raw hashes
    # and sizes and does not treat exact pattern count as physical audibility.
    if not pattern:
        return 0
...
def run(backend: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Cancel before the old sentinel is allowed to reach the sink.
        old_generation, new_generation = fence.cancel()
        cancel_called_ns = time.monotonic_ns()
        sink.cancel()
        cancel_finished_ns = time.monotonic_ns()

        # Deliberately late old-generation chunk must fail at the app fence.
    ...
    # For pulse-null, exact byte count is diagnostic rather than a universal
    # pass condition because server scheduling/resampling may fragment capture.
    pulse_pass = (
        backend != "pulse-null"
        or (
            not late_old_accepted
