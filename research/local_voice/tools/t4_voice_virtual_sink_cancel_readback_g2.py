...
representable FDX9 boundary: an old generation is genuinely queued at a sink,
the generation fence advances *before* cancellation, pending old audio is
quiesced, a late old-generation chunk is rejected, a new generation restarts,
and sink-monitor readback is inspected for stale sentinel leakage.
...
RESEARCH_ID = "T7-20260902-COMPLETION-DEPTH-G2"
SEMANTIC_KEY = "0d83f7d13c1d8f91686cf94070f73d901a49018b40a9c058d1949af094655bff"
CLASSIFICATION = "CANDIDATE_FALSIFIER_FDX9_VIRTUAL_SINK_ONLY"
...
PRE_CHUNKS = 20          # 200 ms of preamble before the stale sentinel.
OLD_SENTINEL_CHUNKS = 40 # 400 ms deliberately queued before cancellation.
NEW_SENTINEL_CHUNKS = 20 # 200 ms must be observed after restart.
CUT_DELAY_S = 0.080      # Cancel while still inside the 200 ms preamble.
...
class PulseNullSink:
    ...
    The old generation is written to pacat before cancellation.  SIGTERM closes
    the playback stream instead of using drain semantics; monitor capture then
    supplies sink-side readback.  This remains virtual-sink evidence only.
    """

    backend_name = "PIPEWIRE_PULSE_NULL_SINK"

    def __init__(self, sink_name: str = "f2_voice_fdx9_g2") -> None:
    ...
    def start(self) -> None:
        ...
        self._tmp = tempfile.TemporaryDirectory(prefix="f2-voice-fdx9-g2-")
...
def _run_with_sink(sink: QueueingSoftwareLoopback | PulseNullSink, *, pulse: bool) -> dict[str, Any]:
    ...
    # Critical discriminator: OLD_SENTINEL is genuinely queued before cancel.
    ...
    if pulse:
        ...
        # Deterministically play 80 ms of the 200 ms preamble; stale sentinel
        # remains pending and therefore cancellation must discard it.
    ...
    # For same-rate null sink, any full-scale old sentinel sample after a cut
    # that occurs 120 ms before the sentinel is scheduled is evidence that
    # queued old-generation audio leaked through cancellation.
    passed = (
    ...
    if not pulse:
        passed = passed and new_samples == expected_new_samples
    ...
    return {
        ...
        "execution_scope": "VPS_VIRTUAL_SINK_CANDIDATE" if pulse else "REPOSITORY_REFERENCE_ONLY",
        ...
        "evidence": {
            ...
            "pulse_virtual_sink_promotion_candidate": pulse_runtime_candidate,
        ...
        "next_exact_action": (
            ...
            "bind exact source/runtime/monitor-readback in an external receipt before promoting bounded FDX9 virtual-sink credit. "
            "Keep producer-side TTS cancel and true partial ASR as separate G2 discriminators; reserve physical speaker/mic/human-heard for S4."
        ),
    }
