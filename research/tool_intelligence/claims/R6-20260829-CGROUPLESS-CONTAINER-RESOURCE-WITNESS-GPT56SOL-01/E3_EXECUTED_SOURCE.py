...
def _parse_stat(text: str) -> dict[str, int | str]:
    # comm is parenthesized and may contain spaces or ')' characters. rfind is
    # correct for the kernel's final closing parenthesis before state.
    lparen = text.find("(")
    rparen = text.rfind(")")
...
def witness(pid: int, interval_seconds: float) -> dict[str, Any]:
    ...
    try:
        ...
        result: dict[str, Any] = {
            ...
            "pid": pid,
            "process_identity": {
                "starttime_ticks": identity,
                "pidfd_state": pidfd_state,
            },
