> **⚠️ SUPERSEDED by v2, same day (2026-09-03).** Gabriel read this v1 draft and
> precisified it directly: §7's four open questions are now answered, §4's
> "child of HostIdentity" framing is replaced (StateRootIdentity belongs to
> InstallationIdentity; the host relationship becomes a `HostBinding`
> attestation, not a parent), and §5's session-id framing is corrected
> (RuntimeEpoch is defined by continuous runtime lifecycle, session id is
> evidence only). This file is kept verbatim, unedited, as the historical
> record of what was proposed before Gabriel's correction — per this blog's
> transparency principle, nothing here is rewritten retroactively. The
> current model, exact target schema, and the tested code implementing it
> are in
> `ENTITY_IDENTITY_LAYERING_20260903_V2.md` and
> `src/frankenstein2/entity_identity.py` (paket-1788425913354-5072e0).
