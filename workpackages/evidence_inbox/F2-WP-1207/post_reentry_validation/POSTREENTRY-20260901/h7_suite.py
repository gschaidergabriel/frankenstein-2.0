...
def run_test(t: dict, phase: str) -> dict:
    ...
    try:
        ...
        answer, num_turns, err = "", None, None
    ...
    return dict(test_id=t["id"], capability=t["cap"], pass_=all(c["pass_"] for c in crits),
                ...
                error=err, ts_utc=time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))
...
def main() -> None:
    ...
    args = ap.parse_args()
    res = [run_test(t, args.phase) for t in tests()
