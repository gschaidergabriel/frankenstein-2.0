...
Frankenstein 2.0 completion also requires the permission-gated multi-source Perception Fabric defined in `architecture/PERCEPTION_FABRIC.md`, `architecture/PERCEPTION_FABRIC_HARDENING_20260829.md` and `workpackages/PERCEPTION_FABRIC_PHASE.json`.
...
- `0..N` configured/permitted sources without assuming four sources;
- one capture owner per source with bounded RAM fan-out;
- continuous cheap Retina L0 without required LLM/VLM-token use;
- GRID10-driven `ObserveIntent` active sensing with exact permission-snapshot binding;
- a dynamically admitted `0..4` Retina/Cortex analysis-worker pool initially, independent of source count;
- local OCR/UI/DOM/AX/tracking/specialist analysis before generic VLM escalation;
- temporal/clock-domain-safe and multi-view fusion into epistemically typed world-model updates;
...
- bounded cardinality/time/revocation/resource falsifiers and a four-source no-general-VLM soak when four sources can be provided or synthesized.
